...
function services($category= '') {
	...
	private function reload_live_fluxsbc($command)
	{
		$response = '';
		...
		foreach ($fs_data as $fs_key => $fs_value) {
		    $fp = $this->fluxsbc_lib->event_socket_create($fs_value["fluxsbc_host"], $fs_value["fluxsbc_port"], $fs_value["fluxsbc_password"]);
		    if ($fp) {
			$response .= $this->fluxsbc_lib->event_socket_request($fp, $command);
			fclose($fp);
		    }
		}
	...
	private function check_calls_running(){
		 ...
		 $response = $this->reload_live_fluxsbc($command);
		 $calls = array();
		 $calls_final = array();
		 $data_header = array();
		 $k = 0;

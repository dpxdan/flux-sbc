...
// ##############################################################################
class Flux SBC extends MX_Controller
{
    ...
    {
        $this->permission->check_web_record_permission($edit_id, 'sip_devices', 'fluxsbc/fssipdevices/');
    ...
    function fsgateway_search()
    {
        ...
        if (@$ajax_search != 1) {
            redirect(base_url() . 'fluxsbc/fsgateway/');
        }
    }
    ...
    function fssipprofile_search()
    {
        ...
        if (@$ajax_search != 1) {
            redirect(base_url() . 'fluxsbc/fssipprofile/');
        }
    }
    ...
    function fssipdevices_save($user_flg = false)
    {
        ...
        if ($add_array['id'] != '') {
            $data['page_title'] = gettext('Flux SBC SIP Devices');
            ...
            $data['page_title'] = gettext('Create Flux SBC SIP Devices');
            if ($this->form_validation->run() == FALSE) {
                $data['validation_errors'] = validation_errors();
                echo $data['validation_errors'];
                exit();
    ...
    function customer_fssipdevices_save($user_flg = false)
    {
        ...
        if ($add_array['id'] != '') {
            $data['page_title'] = gettext('Edit Flux SBC SIP Devices');
            if ($this->form_validation->run() == FALSE) {
                $data['validation_errors'] = validation_errors();
                echo $data['validation_errors'];
                exit();
    ...
    function fssipdevices_search()
    {
        ...
        if (@$ajax_search != 1) {
            redirect(base_url() . 'fluxsbc/fssipdevices/');
        }
    }
    ...
    function fssipdevices_json()
    {
        ...
        foreach ($query as $key => $value) {
            ...
            $current_row = array(
                $checkbox,
                "<a href='/fluxsbc/fssipdevices_edit/" . $value['id'] . "' style='cursor:pointer;color:#005298;' rel='facebox_medium' title='username'>" . $value['username'] . "</a>",
                $value['password'],
    ...
    function get_action_fssipdevices_buttons($id, $accountid, $entity_type = '')
    {
        ...
        if ($this->session->userdata("logintype") == '0' || $this->session->userdata("logintype") == '3') {
            ...
            if (isset($permissioninfo['fluxsbc']['fssipdevices']['EDIT']) and $permissioninfo['fluxsbc']['fssipdevices']['EDIT'] == 0 and ($permissioninfo['login_type'] == '1' or $permissioninfo['login_type'] == '2' or $permissioninfo['login_type'] == '4') or $permissioninfo['login_type'] == '-1') {
            ...
            if (isset($permissioninfo['fluxsbc']['fssipdevices']['DELETE']) and $permissioninfo['fluxsbc']['fssipdevices']['DELETE'] == 0 and ($permissioninfo['login_type'] == '1' or $permissioninfo['login_type'] == '2' or $permissioninfo['login_type'] == '4') or $permissioninfo['login_type'] == '-1') {
    ...
    function get_action_buttons_fssipdevices($id)
    {
        ...
        $ret_url = '';
        if (isset($permissioninfo['fluxsbc']['fssipdevices']['EDIT']) and $permissioninfo['fluxsbc']['fssipdevices']['EDIT'] == 0 and ($permissioninfo['login_type'] == '1' or $permissioninfo['login_type'] == '2' or $permissioninfo['login_type'] == '4') or $permissioninfo['login_type'] == '-1') {
            $ret_url = '<a href="' . base_url() . 'fluxsbc/fssipdevices_edit/' . $id . '/" class="btn btn-royelblue btn-sm"  rel="facebox_medium" title="Edit">&nbsp;<i class="fa fa-pencil-square-o fa-fw"></i></a>&nbsp;';
        }
        if (isset($permissioninfo['fluxsbc']['fssipdevices']['DELETE']) and $permissioninfo['fluxsbc']['fssipdevices']['DELETE'] == 0 and ($permissioninfo['login_type'] == '1' or $permissioninfo['login_type'] == '2' or $permissioninfo['login_type'] == '4') or $permissioninfo['login_type'] == '-1') {
            $ret_url .= '<a href="' . base_url() . 'fluxsbc/fssipdevices_delete/' . $id . '/" class="btn btn-royelblue btn-sm" title="Delete" onClick="return get_alert_msg();">&nbsp;<i class="fa fa-trash fa-fw"></i></a>';
        }
        return $ret_url;
    }
    ...
    function livecall_report_json()
    {
        ...
        $response = $this->fluxsbc_model->reload_live_fluxsbc($command);
        $calls = array();
        $calls_final = array();
        $data_header = array();
        $k = 0;
        ...
        foreach ($calls as $key => $value) {
            if (isset($value['state']) && ($value['state'] == 'CS_EXCHANGE_MEDIA' || $value['state'] == 'CS_CONSUME_MEDIA')) {
                ...
                $json_data['rows'][] = array(
                    'cell' => array(
                        "<a href='" . base_url() . "fluxsbc/livecall_hangup?uuid=" . $value['uuid'] . "' class='btn btn-warning'> Hang Up </button>",
	...
	if(!empty($uuid)){
        ...
        redirect(base_url() . 'fluxsbc/livecall_report/');
    }
        ...
        if ($gateway_data['id'] != '') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($update) {
                    ...
                    $this->fluxsbc_model->reload_fluxsbc($cmd, $sip_ip);
                    $cmd2 = "api sofia profile " . $sip_profile_info['name'] . " rescan reloadacl reloadxml";
                    $this->fluxsbc_model->reload_fluxsbc($cmd2, $sip_ip);
                }
                echo json_encode(array(
    ...
    function fsgateway_delete_multiple()
    {
        ...
        foreach ($gateway_info as $key => $value) {
            ...
            $this->fluxsbc_model->reload_fluxsbc($cmd, $sip_profile_arr[$value['sip_profile_id']]['sip_ip']);
        }
        $this->db->where($where);
        ...
        foreach ($gateway_result as $key => $value) {
            $json_data['rows'][] = array(
                'cell' => array(
                    ...
                    $value,
                    array(
                        '<a href="/fluxsbc/fssipprofile_edit/' . $edited_id . '/edit/' . $key . '/" class="btn btn-royelblue btn-sm"  title="Edit">&nbsp;<i class="fa fa-pencil-square-o fa-fw"></i></a>&nbsp;<a href="/fluxsbc/fssipprofile_delete_params/' . $edited_id . '/' . $key . '/" class="btn btn-royelblue btn-sm" title="Delete" onClick="return get_alert_msg();">&nbsp;<i class="fa fa-trash fa-fw"></i></a>'
                    )
                )
            );
        }
    ...
    function fssipprofile_action($button_name, $id)
    {
        ...
        $this->fluxsbc_model->reload_fluxsbc($cmd);
        ...
        if ($add == 'add') {
            ...
            if ($query > 0) {
                ...
                if ($sipprofile_data['name'] == '' || $sipprofile_data['sip_ip'] == '' || $sipprofile_data['sip_port'] == '') {
                    ...
                    redirect(base_url() . 'fluxsbc/fssipprofile_add/');
                    exit();
                }
    ...
    function fssipprofile_edit($edit_id = '', $type = '', $name_prams = '')
    {
        ...
        if ($type == 'edit' || isset($sipprofile_data['type']) && $sipprofile_data['type'] == 'save') {
            ...
            if (isset($sipprofile_data['type']) && $sipprofile_data['type'] == 'save') {
                ...
                redirect(base_url() . 'fluxsbc/fssipprofile_edit/' . $sipprofile_data['id']);
                exit();
            }
        }
    ...
    function fssipprofile_delete_params($id, $name)
    {
        ...
        redirect(base_url() . 'fluxsbc/fssipprofile_edit/' . $id);
    }

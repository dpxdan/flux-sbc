#!/bin/bash
#########################################
# Script de instalação do Flux SBC      #
# Versão 6.0                            #
# Autor: Daniel Paixao                  #
# Supervisão: Diego Menine              #
# Data: 22/06/2022						#
# http://flux.net.br                    #
#########################################
...
{ 
        ...
        if [[ $V1 = "Debian GNU/Linux 9" ]]; then
                ...
                DIST="OTHER"
                echo -e 'Ooops!!! Quick Installation does not support your distribution \nPlease use manual steps or contact FLUX Dev Team \nat daniel@flux.net.br.'
                exit 1
        fi
        fi
        fi
...
{
        cd /opt
        git clone https://github.com/dpxdan/flux-sbc.git flux
}
...
license_accept ()
{
        ...
        if [ $IS_ENTERPRISE = "False" ]; then
                ...
                if [ -f LICENSE ]; then
                        ...
                        wget --no-check-certificate -q -O GNU-AGPLv5.0.txt https://raw.githubusercontent.com/dpxdan/flux-sbc/main/LICENSE
...
get_user_response ()
{
        ...
        curl --data "email=$EMAIL" --data "data=$NAT2" --data "type=Install" https://dev.flux.net.br/lib/
}

...
$(document).ready(function(){
  $("#fluxsbc_command").change(function(){
...
function validateForm(){
      if(document.getElementById('fluxsbc_command').value == "")
      {
	  $('#error_field_command').text( "<?php echo gettext('Please Enter Switch command'); ?>" );
	  document.getElementById('fluxsbc_command').focus();
	  return false;
      }
      $('#form').submit();
}
			...
			<div class="row">
				<div class="col-md-12">
					<form method="POST" id="form" name="form"
						action="<?php echo base_url(); ?>fluxsbc/fluxsbc_fs_cli_command/"
        ...
        foreach ($fs_data as $name) {
            ?>
     			 <option value="<?= $name['id'] ?>"
										<?php if(isset($host_id) && ($name['id'] == $host_id))echo 'selected';?>><?= $name['fluxsbc_host'] ?></option>
  			  <?php
        }
        ?>
		   </select>
							...
							<div class="row">
								<div class="col-md-3">
									<input type="text" class="form-control"
										value="<?php echo $command_show; ?>" id="fluxsbc_command"
										name="fluxsbc_command" placeholder="fluxsbc command"
										style="height: 40px;">
								</div>

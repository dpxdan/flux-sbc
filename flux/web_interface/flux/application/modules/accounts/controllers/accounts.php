...
class Accounts extends MX_Controller
{
    ...
    function customer_ipmap_action($action, $accountid, $accounttype, $ipmapid = "")
    {
            ...
            if ($add_array['ip'] != "") {
                ...
                if ($getdata > 0) {
                    ...
                    if ($ip_flag) {
                        ...
                        $this->load->module('fluxsbc/fluxsbc');
                        $command = "api reloadacl";
                        $response = $this->fluxsbc_model->reload_fluxsbc($command);
        ...
        if ($action == "delete") {
            ...
            $command = "api reloadacl";
            $this->fluxsbc_model->reload_fluxsbc($command);
    ...
    function customer_details_json($module, $accountid)
    {
        ...
        if ($module == "fluxsbc") {
            $this->load->module('fluxsbc/fluxsbc');
            $this->fluxsbc->customer_fssipdevices_json($accountid, $entity_type);
        }
        if ($module == "did") {
    ...
    function customer_sipdevices($edit_id)
    {
        ...
        if (($account_res->num_rows() > 0) || ($accountinfo['type'] = - 1)) {
            ...
            $accounttype = strtolower($this->common->get_entity_type('', '', $account_data['type']));
            $this->load->module('fluxsbc/fluxsbc');
            $data["grid_buttons"] = $this->fluxsbc->fluxsbc_form->fsdevices_build_grid_buttons($edit_id, $accounttype);
            $data['grid_fields'] = $this->fluxsbc->fluxsbc_form->build_devices_list_for_customer();
            $data['edit_id'] = $edit_id;
            $data['accounttype'] = $accounttype;
    ...
    function customer_fssipdevices_action($action, $id, $accountid)
    {
        ...
        $this->load->module('fluxsbc/fluxsbc');
        if ($action == "delete") {
            $this->fluxsbc->fluxsbc_model->delete_freeswith_devices($id);
        ...
        if ($action == "edit") {
            $this->fluxsbc->customer_fssipdevices_edit($id, $accountid);
